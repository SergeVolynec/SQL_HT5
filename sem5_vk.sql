...
CREATE OR REPLACE VIEW friends_1 AS 
SELECT	
	fr.initiator_user_id AS friend
FROM 
	friend_requests fr
WHERE
	fr.target_user_id = 1 AND fr.status = 'approved'
UNION
SELECT	
	fr.target_user_id AS friend
FROM 
	friend_requests fr
WHERE
	fr.initiator_user_id = 1 AND fr.status = 'approved';
...
SELECT 
	id,
    created_at,
    LEAD (created_at) OVER (ORDER BY created_at) AS created_at_next,
    TIMEDIFF(LEAD (created_at) OVER (ORDER BY created_at),created_at) AS 'waiting_time'
FROM messages
